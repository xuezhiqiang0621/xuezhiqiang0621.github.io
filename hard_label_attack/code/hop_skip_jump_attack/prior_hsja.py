...
class PriorHopSkipJumpAttack(object):
    def __init__(self, model, surrogate_model, dataset,  clip_min, clip_max, height, width, channels, norm, load_random_class_image, epsilon,
    ...
    def cw_loss(self, logit, label, target=None):
        if target is not None:
            ...
            target_is_max = argsort[:, 0].eq(target.cuda()).long()
    ...
    def approximate_gradient_with_prior(self, sample, true_labels, target_labels, perturb_sample_num, delta):
        with torch.enable_grad():
            # sample shape = [1, C,H,W]
            sample_ = sample.detach().cuda()
            sample_.requires_grad = True
            surrogate_logits = self.surrogate_model(sample_)
            surrogate_loss = self.cw_loss(surrogate_logits, true_labels, target_labels)
            prior_grad = torch.squeeze(torch.autograd.grad(surrogate_loss, sample_, retain_graph=False)[0]).cpu().detach() # C,H,W
        # 1: adv region, -1: non-adv region
        prior_deriv = 2 * self.decision_function(torch.clamp(sample + delta * self.normalize(prior_grad).unsqueeze(0),
                                                             self.clip_min,self.clip_max),
                                                 true_labels, target_labels).float().view(-1).item() - 1
        total_num_eval = 1
        if prior_deriv < 0:
            prior_deriv, prior_grad = -prior_deriv, -prior_grad

        # Generate random vectors.
        noise_shape = [perturb_sample_num] + list(self.shape)
        if self.norm == 'l2':
            rv = torch.randn(*noise_shape)
        elif self.norm == 'linf':
            rv = torch.from_numpy(np.random.uniform(low=-1, high=1, size=noise_shape)).float()  # FIXME 是否要这样写，看看实验结果
        us = []
        us.extend(rv)
        ...
        total_num_eval += perturbed.size(0)
    ...
    def attack(self, batch_index, images, target_images, true_labels, target_labels):
        ...
        for j in range(self.num_iterations):
            ...
            num_perturb_samples = int(self.init_num_evals * np.sqrt(j+1))
            num_perturb_samples = int(min([num_perturb_samples, self.max_num_evals]))
            gradf, est_grad_num_eval = self.approximate_gradient_with_prior(perturbed.unsqueeze(0), true_labels, target_labels,
                                                                         num_perturb_samples, delta)
            query += est_grad_num_eval
...
def get_exp_dir_name(dataset,  norm, targeted, target_type, random_direction, args):
    ...
    if args.init_num_eval_grad!=100:
        if args.attack_defense:
            dirname = 'PriorHSJA@{}_on_defensive_model-{}-{}-{}'.format(args.init_num_eval_grad, dataset, norm, target_str)
        else:
            dirname = 'PriorHSJA@{}-{}-{}-{}'.format(args.init_num_eval_grad, dataset, norm, target_str)
    else:
        if random_direction:
            if args.attack_defense:
                dirname = 'PriorHSJARandom_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
            else:
                dirname = 'PriorHSJARandom-{}-{}-{}'.format(dataset, norm, target_str)
        else:
            if args.attack_defense:
                dirname = 'PriorHSJA_on_defensive_model-{}-{}-{}'.format(dataset, norm, target_str)
            else:
                dirname = 'PriorHSJA-{}-{}-{}'.format(dataset, norm, target_str)
    return dirname
...
if __name__ == "__main__":
    ...
    parser.add_argument('--surrogate-arch', required=True, type=str, help='network architecture')
    ...
    print_args(args)
    surrogate_model = StandardModel(args.dataset, args.surrogate_arch, no_grad=False, load_pretrained=True).cuda()
    ...
    for arch in archs:
        ...
        model.cuda()
        model.eval()
        attacker = PriorHopSkipJumpAttack(model, surrogate_model, args.dataset, 0, 1.0, model.input_size[-2], model.input_size[-1], IN_CHANNELS[args.dataset],
